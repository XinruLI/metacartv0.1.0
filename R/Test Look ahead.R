REmrt.data <- function(formula, data, vi, c = 1, maxL = 5L, minsplit = 2L, delQ = 0.001, minbucket = 3, n.fold = 10, ...){
  Call <- match.call()
...
mf <- REmrt.data(efk~m1+m2+m3+m4+m5, vi = vark, data = SimData)
mf2 <- REmrt.data(g ~ T1+ T2+ T4+ T25, vi = vi, data = dat.BCT2009)
res.test <- REmrt.fit0.ahead(mf2, 5, 3, 2)
res2 <- REmrt.fit0(mf2,5,3)
res2$tree
res.test$tree
